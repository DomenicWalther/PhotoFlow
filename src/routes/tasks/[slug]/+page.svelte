    ...
    <div class="border-t border-gray-200">
      <dl>
        ...
        {#each data.comments as comment }
                <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt class="text-sm font-medium text-gray-500">{comment.createdAt.toLocaleDateString('de-DE', dateOptions)}</dt>
                  <dd class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">{comment.comment}</dd>
                </div>
                {/each}
      </dl>
    </div>
